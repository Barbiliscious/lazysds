...
export const FIELD_SPECS: FieldSpec[] = [
  ...
  { key: "issue_date", column: 7, header: "Issue / Revision Date", guidance: "Most recent of issue or revision. YYYY-MM-DD, or YYYY-MM if only month given, or YYYY. The print date is NOT the issue date. Never invent a day." },
  { key: "review_date_stated", column: 8, header: "Review Date (stated)", guidance: "ONLY if the SDS states a review-by date. If it doesn't, status NOT_STATED (the app calculates Issue + 5 years itself)." },
...
export function fieldCellText(field: SDSField): string {
  if (field.status === "STATED" && field.value) return field.value;
