...
/**
 * How sure the source is that this is the right product. Set by the AI for a
 * web-search match; a saved mapping or a direct barcode-database hit is
 * always "high" - either a human already confirmed it, or the barcode itself
 * was an exact key into a database, not a fuzzy guess.
 */
export type BarcodeConfidence = "high" | "medium" | "low";

/**
 * A candidate product for a scanned barcode, from whichever source answered
 * it (a saved mapping, one of the barcode-database providers, or the AI
 * web-search fallback). Structured rather than one combined name so /find can
 * build a precise SDS search from the strongest identifiers available, and so
 * the worker can review/correct individual fields before it's saved as a
...
export interface ScannedProduct {
  ...
  name: string;
  brand: string | null;
  /** The manufacturer's own product/item code - never the barcode itself. */
  manufacturerProductCode: string | null;
  /** Pack size / quantity, e.g. "500g". */
  size: string | null;
  /** Colour / scent / type variant, e.g. "Matt Black". */
  variant: string | null;
  /** Best source URL backing this answer, if any. */
  sourceUrl: string | null;
  /** Which path found it: 'saved_mapping', a provider id, or 'web_search'. */
  sourceProvider: string | null;
  confidence: BarcodeConfidence | null;
}

/** A worker-confirmed barcode -> product mapping, stored for instant reuse. */
