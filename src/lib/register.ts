...
/**
 * Saves edits to an existing entry. The date-derived columns (review date,
 * currency) are recomputed in code the same way as at first save, so a
 * corrected Issue Date flows through everywhere. The PDF, its URL, the SDS
 * Record ID and SDS Filename are kept as-is - record_id and filename_stem
 * are immutable after first save (also enforced server-side, see migration
 * 0005) since they name an already-distributed PDF/SharePoint link.
 */
export async function updateRecord(
  ...
  const rebuilt = buildRecord(extracted, record.pdf_url, record.source, verifiedBy, record.filename_stem);
